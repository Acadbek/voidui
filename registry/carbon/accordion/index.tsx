...
const AccordionItem = React.forwardRef<HTMLDivElement, AccordionItemProps>(
  ({ className, ...props }, ref) => (
    <AccordionPrimitive.Item
      ref={ref}
      className={cn("border-t [&:last-child]:border-b", className)}
      {...props}
    />
  ),
);
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 font-medium h-10 px-4 items-center transition-colors justify-between text-sm [&[data-state=open]>svg]:rotate-180",
        "focus:outline-none",
